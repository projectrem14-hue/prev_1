...
export default function Insights() {
  ...
  useEffect(() => {
    document.title = "GapLogic — Progress Portal";
  }, []);
        ...
        <main className="flex-1 md:ml-64 p-6 lg:p-12 pb-24 md:pb-12">
          {intentions.length < 5 ? (
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <p className="text-muted-foreground mb-8 max-w-md">Detailed progress analytics unlock after 5 intentions are logged in your stack.</p>
              <Link href="/modeler"><Button size="lg" className="rounded-2xl px-12 h-14 font-bold shadow-xl shadow-primary/20">Establish Your First Stack</Button></Link>
            </div>
          ) : (
            <>
              <header className="mb-10">
                <h1 className="font-headline text-4xl font-bold tracking-tight mb-2">Progress Portal</h1>
                <p className="text-muted-foreground text-lg">Detailed behavioral trends and performance metrics.</p>
              </header>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <Card className="glass-card border-none">
                  <CardHeader><CardTitle className="font-headline flex items-center gap-2"><TrendingUp className="w-5 h-5 text-primary" /> Integrity Growth</CardTitle></CardHeader>
                  <CardContent className="h-[350px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={dailyTrend}>
                        ...
                        <Area type="monotone" dataKey="rate" stroke="hsl(var(--primary))" fill="url(#colorRate)" strokeWidth={3} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </CardContent>
                </Card>
                <Card className="glass-card border-none">
                  <CardHeader><CardTitle className="font-headline flex items-center gap-2"><Brain className="w-5 h-5 text-primary" /> Consistency Volume</CardTitle></CardHeader>
                  <CardContent className="h-[350px]">
                    <ResponsiveContainer width="100%" height="100%">
